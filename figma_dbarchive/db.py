...
@click.command()
@click.argument("mode", type=click.STRING, default="sync") # command mode - 'sync' / 'populate' (populate mode is used when you want to process deeper in second entry, when first entry is processed with samples)
...
def main(mode, src, db, concurrency, depth, max, shuffle, gc):
    ...
    dbthread = threading.Thread(target=dbworker, args=(db_queue, db, PBARPOS - 1))
    dbthread.start()
